...
const submit = async () => {
    ...
    if(await loginService.generateOtp(dataState.email)) {
        navigateTo({
            path: '/login/otp',
            query: {email: dataState.email}
        })
    }
}
</script>
        ...
        <form class="w-full flex flex-col pt-4 gap-y-3">
            ...
            <div class="space-y-5 py-8 w-full">
                <button @click="submit" type="button"
                        class="bg-orange-300 dark:bg-orange-500 rounded-md p-2 font-semibold text-white w-full">Next
                </button>
                <button type="button"
